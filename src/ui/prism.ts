...
export function createPrism(container: HTMLElement, opts: PrismOptions = {}): () => void {
  const {
    height = 3.5,
    baseWidth = 5.5,
  ...
  const resize = () => {
    ...
    ;(program.uniforms as any).uPxScale.value =
  ...
  if (animationType === 'hover') {
    pointerMoveHandler = (e) => { onMove(e); startRAF() }
    window.addEventListener('pointermove', pointerMoveHandler, { passive: true })
    window.addEventListener('mouseleave', onLeave)
    ;(program.uniforms as any).uUseBaseWobble.value = 0
  } else if (animationType === '3drotate') {
    ;(program.uniforms as any).uUseBaseWobble.value = 0
  } else {
    ;(program.uniforms as any).uUseBaseWobble.value = 1
  }

  function render(t: number) {
    const time = (t - t0) * 0.001
    ;(program.uniforms as any).iTime.value = time
    ...
    if (animationType === 'hover') {
      ...
      ;(program.uniforms as any).uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf)
      if (NOISE_IS_ZERO) {
      ...
      ;(program.uniforms as any).uRot.value = rotBuf
      if (TS < 1e-6) continueRAF = false
    }

    renderer.render({ scene: mesh })
    rafId = continueRAF ? requestAnimationFrame(render) : 0
  }
